...
uint16_t remote_port = 53468;
uint16_t server_port = 53468;
bool tcp_mode = false;

size_t tcp_buffer_size = 256;
size_t tcp_receive_threshold = 128;
...
void UDPLEDBroadcast(SOCKET sHost, const char* memory)
{
    ...
    while(!EXIT_FLAG)
    {
        ...
        current_status.assign(reinterpret_cast<const char*>(memory + 15 + 4 + 2), 6 * 3);
        bool same = true;
...
void InputReceive(SOCKET sHost, char *memory)
{
    ...
    while(!EXIT_FLAG)
    {
        ...
        if(real_len >= 3 + 4 + 15 + 4 && buffer[1] == 'I' && buffer[2] == 'N' && buffer[3] == 'P')
        {
            memcpy(memory, buffer + 4 + 4, 15);
            lever = ntohf(buffer + 4 + 4 + 15);
            memcpy(memory + 15, &lever, 4);
        ...
        if(real_len >= 10 && buffer[1] == 'C' && buffer[2] == 'O' && buffer[3] == 'N')
        {
            std::string data;
            data.assign(buffer + 4, 19);
...
{
    printf("=================================================\n");
    printf("=          Brokengeki-Evolved-Android:          =\n");
    printf("=     Brokengeki with full IO over network      =\n");
...
{
    checkArgs(argc, argv);
    SetConsoleTitle("Brokengeki-Evolved-Android Server");
    printInfo();
    ...
    const char *memFileName = "Local\\BROKENGEKI_SHARED_BUFFER";
